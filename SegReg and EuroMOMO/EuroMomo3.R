...
mortality <- read_excel("Mortality EW final.xlsx", sheet="Summary")
...
EuroMOMO <- function(country, fitstart, fitend, predyear, gender, type){
  ...
  mod <- glm(Mu ~ log(N) + X + WeekNumber + Y1 + Y2, 
...
out <- EuroMOMO("EW", 2013, 2018, 2020, "Female", "Cohort")
