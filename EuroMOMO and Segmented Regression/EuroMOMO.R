...
out <- EuroMOMO("EW", 2015, 2019, 2015, 2019, "All")

ggplot() + 
  geom_vline(xintercept=ymd(c("2016-01-01", "2017-01-01", "2018-01-01", "2019-01-01")), linetype="dashed", color="grey70") +
  geom_line(data=out, aes(WeekEnding, `Predicted deaths`, color="Baseline")) +
  ...
  labs(x = "Date",
       y = "Deaths",
       title = "England and Wales Model-Fit for 2015-2019") +
  scale_color_manual(name="Legend",
                     values=c("red4", "red1"),
                     limits = c("Baseline", "Actual deaths")) +
  theme_minimal()
...
# All UK nations model #############################################################

out <- EuroMOMO("EW", 2015, 2019, 2020, 2020, "All")
...
EW <- ggplot() +
  geom_line(data = deaths2019, aes(WeekNumber, Deaths, color = "Deaths 2017-2019"), show.legend=FALSE) +
  geom_line(data = deaths2018, aes(WeekNumber, Deaths, color = "Deaths 2017-2019"), show.legend=FALSE) +
  geom_line(data = deaths2017, aes(WeekNumber, Deaths, color = "Deaths 2017-2019"), show.legend=FALSE) +
  geom_line(data=out, aes(WeekNumber, `Actual deaths`, color = "Actual deaths"), show.legend=FALSE) +
  geom_line(data=out, aes(WeekNumber, `Predicted deaths`, color = "Baseline deaths"), show.legend=FALSE) +
#  geom_hline(yintercept = 0) +
  labs(x = "Week number",
       y = "Deaths",
       title = "England and Wales 2020") +
  scale_color_manual(name="Legend",
...
# 
# out1 <- EuroMOMO("EW", 2015, 2019, 2020, 2020, "Female")
# out2 <- EuroMOMO("EW", 2015, 2019, 2020, 2020, "Male")
# 
# EWG <- ggplot() +
...
out <- EuroMOMO("S", 2015, 2019, 2020, 2020, "All")
...
out <- EuroMOMO("NI", 2015, 2019, 2020, 2020, "All")
...
grid.arrange(EW, S, NI, nrow=1,ncol=3, top=textGrob("EuroMOMO: 2020 Model for All UK Nations"))

# Table for comparing cumulative excess deaths ############################

out <- EuroMOMO("EW", 2015, 2019, 2020, 2020, "All")
out <- rbind(out, EuroMOMO("EW", 2015, 2019, 2021, 2021, "All"))

out <- out[, c("Year", "WeekNumber", "Actual deaths", "Predicted deaths")]

write.csv(out, "EuroMOMO comparison.csv")
