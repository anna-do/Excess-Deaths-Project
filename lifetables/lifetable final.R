...
iterate <- function(Country, PopStructure, LifeTable, BirthRateYear, Sex, Iterations){
  ...
  for(i in 2:ncol(Surviving)){ #Looping over weeks, starting at i=2 so week 1
    ...
    Surviving[1, i] <- Surviving[1, i] + BirthRate # newborns
  }
...
## Sanity check using 2017-2019 lifetables and 2017-2019 pop
...
testEW_female <- pred/real
testEW_female <- (testEW_female-1)*100
print(testEW_female) # Overestimating deaths by 0.9% for females in E&W

# male
testE <- iterate("England", 2017, 2017, 2017, "male", 3)
testW <- iterate("Wales", 2017, 2017, 2017, "male", 3)

pred <- sum(testE$Deaths) + sum(testW$Deaths)
real <- sum(actualdeaths[actualdeaths$Year <= 2019 &
                           actualdeaths$Year >=2017 &
                           actualdeaths$Country == "EW" &
                           actualdeaths$Gender == "Male", "Deaths"])
testEW_male <- pred/real
testEW_male <- (testEW_male-1)*100
print(testEW_male) # Overestimating deaths by 2.42% for males in E&W

# scotland all
testS_female <- iterate("Scotland", 2017, 2017, 2017, "female", 3)
testS_male <- iterate("Scotland", 2017, 2017, 2017, "male", 3)

pred <- sum(testS_female$Deaths) + sum(testS_male$Deaths)
real <- sum(actualdeaths[actualdeaths$Year <= 2019 &
                           actualdeaths$Year >=2017 &
                           actualdeaths$Country == "S" &
                           actualdeaths$Gender == "All", "Deaths"])

testS <- pred/real
testS <- (testS-1)*100
print(testS) # Overestimating deaths by 2.24% for all in scotland
...
LT <- function(Country, PopStructure, LifeTable, BirthRateYear, Sex, Iterations){
  ...
  PredDeaths <- gamout$s.w. + baseline
...
# Filter out rows where Year is 2020
outEW_2020 <- out[out$Year == 2020, ]
# Sum the Excess column for the filtered rows
total_EWexcess_2020 <- sum(outEW_2020$Excess, na.rm = TRUE)
print(total_EWexcess_2020)


outEW_2021 <- out[out$Year == 2021, ]
total_EWexcess_2021 <- sum(outEW_2021$Excess, na.rm = TRUE)
print(total_EWexcess_2021)
...
# Filter out rows where Year is 2021
outS_2020 <- out[out$Year == 2020, ]
# Sum the Excess column for the filtered rows
total_Sexcess_2020 <- sum(outS_2020$Excess, na.rm = TRUE)
# Print the total excess for 2021
print(total_Sexcess_2020)

# Filter out rows where Year is 2021
outS_2021 <- out[out$Year == 2021, ]
total_Sexcess_2021 <- sum(outS_2021$Excess, na.rm = TRUE)
print(total_Sexcess_2021)
...
# additional plots for excess deaths
out <- LT("EW", 2020, 2017, 2020, "All", 2)
EW_LT_excess <- ggplot(out, aes(x = WeekEnding, y = Excess)) +
  geom_bar(stat = "identity", fill = "red") +
  labs(title = "Excess Deaths in 2020 to 2021 for England and Wales",
       x = "Week Ending",
       y = "Number of Excess Deaths") +
  theme_minimal()
ggsave("lifetable_ew_excess.png", EW_LT_excess, bg = "white")



out <- LT("S", 2020, 2017, 2020, "All", 2)
S_LT_excess <- ggplot(out, aes(x = WeekEnding, y = Excess)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(title = "Excess Deaths in 2020 to 2021 for Scotland",
       x = "Week Ending",
       y = "Number of Excess Deaths") +
  theme_minimal()
ggsave("lifetable_s_excess.png", S_LT_excess, bg = "white")


outF <- LT("EW", 2020, 2017, 2020, "Female", 2)
EW_F_excess <- ggplot(out, aes(x = WeekEnding, y = Excess)) +
  geom_bar(stat = "identity", fill = "hotpink") +
  labs(title = "Female Excess Deaths in 2020 to 2021 for England and Wales",
       x = "Week Ending",
       y = "Number of Excess Deaths") +
  theme_minimal()
ggsave("lifetable_female_excess.png", EW_F_excess, bg = "white")

outM <- LT("EW", 2020, 2017, 2020, "Male", 2)
EW_M_excess <- ggplot(out, aes(x = WeekEnding, y = Excess)) +
  geom_bar(stat = "identity", fill = "steelblue1") +
  labs(title = "Male Excess Deaths in 2020 to 2021 for England and Wales",
       x = "Week Ending",
       y = "Number of Excess Deaths") +
  theme_minimal()
ggsave("lifetable_male_excess.png", EW_M_excess, bg = "white")
